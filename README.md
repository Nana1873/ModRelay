...
- visible semantic versions for the running app and available updates
...
- conservative hand-off through Penumbra's supported HTTP API, with safe retry behavior and retained sources when completion cannot be confirmed
...
- By default, import failures and unconfirmed imports show a notification and keep the source file, even when success notifications are disabled.
...
- Archive extraction rejects unsafe Windows file names and stops at 32 GB or before consuming the drive's final 512 MB.
...
- Logs contain local paths and mod file names; review them before sharing.
